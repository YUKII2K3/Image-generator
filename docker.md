...
- A computer with specs good enough to run AI Image Generator, and proprietary Nvidia drivers
...
```sh
docker run -p 7865:7865 -v ai-image-generator-data:/content/data -it \
--gpus all \
-e CMDARGS=--listen \
...
-e path_fooocus_expansion=/content/data/models/prompt_expansion/ai_image_generator_expansion/ \
...
```sh
podman run -p 7865:7865 -v ai-image-generator-data:/content/data -it \
...
Your models and outputs are stored in the `ai-image-generator-data` volume, which, depending on OS, is stored in `/var/lib/docker/volumes/` (or `~/.local/share/containers/storage/volumes/` when using `podman`).
...
When you are using `docker compose up` continuously, the container is not updated to the latest version of AI Image Generator automatically.
Run `git pull` before executing `docker compose build --no-cache` to build an image with the latest AI Image Generator version.
