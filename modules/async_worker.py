...
class AsyncTask:
    def __init__(self, args):
        ...
        self.save_metadata_to_images = args.pop() if not args_manager.args.disable_metadata else MetadataScheme.AI_IMAGE_GENERATOR
        self.metadata_scheme = MetadataScheme(
            args.pop()) if not args_manager.args.disable_metadata else MetadataScheme.AI_IMAGE_GENERATOR
...
def worker():
    ...
    import image_generator_version

    from extras.censor import default_censor
    from modules.sdxl_styles import apply_style, get_random_style, ai_image_generator_expansion, apply_arrays, random_style_name
    ...
    def progressbar(async_task, number, text):
        print(f'[AI Image Generator] {text}')
    ...
    def save_and_log(async_task, height, imgs, task, use_expansion, width, loras, persist_image=True) -> list:
        ...
        for x in imgs:
            d = [('Prompt', 'prompt', task['log_positive_prompt']),
                 ...
                 ('AI Image Generator V2 Expansion', 'prompt_expansion', task['expansion']),
                 ('Styles', 'styles',
                  str(task['styles'] if not use_expansion else [ai_image_generator_expansion] + task['styles'])),
            ...
            d.append(('Version', 'version', 'AI Image Generator v' + image_generator_version.version))
    ...
    def process_prompt(async_task, prompt, negative_prompt, base_model_additional_loras, image_number, disable_seed_increment, use_expansion, use_style,
        ...
        if use_expansion:
            ...
            for i, t in enumerate(tasks):
                ...
                progressbar(async_task, current_progress, f'Preparing AI Image Generator text #{i + 1} ...')
    ...
    def handler(async_task: AsyncTask):
        ...
        if ai_image_generator_expansion in async_task.style_selections:
            use_expansion = True
            async_task.style_selections.remove(ai_image_generator_expansion)
        else:
            use_expansion = False
