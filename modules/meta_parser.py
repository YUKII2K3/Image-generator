    ...
    def to_json(self, metadata: str) -> dict:
        ...
        *lines, lastline = metadata.strip().split("\n")
        ...
        if 'raw_prompt' in data:
            ...
            if metadata_prompt != raw_prompt and modules.sdxl_styles.ai_image_generator_expansion not in found_styles:
                found_styles.append(modules.sdxl_styles.ai_image_generator_expansion)
    ...
    def to_string(self, metadata: dict) -> str:
        ...
        if len(self.loras) > 0:
            ...
            for index, (lora_name, lora_weight, lora_hash) in enumerate(self.loras):
                # workaround for AI Image Generator not knowing LoRA name in LoRA metadata
        ...
        generation_params[self.fooocus_to_a1111['created_by']] = 'Customized and maintained by Yuktheshwar'
...
class AIImageGeneratorMetadataParser(MetadataParser):
    def get_scheme(self) -> MetadataScheme:
        return MetadataScheme.AI_IMAGE_GENERATOR
...
def get_metadata_parser(metadata_scheme: MetadataScheme) -> MetadataParser:
    match metadata_scheme:
        ...
        case MetadataScheme.AI_IMAGE_GENERATOR:
            return AIImageGeneratorMetadataParser()
        case _:
            # Fallback for any legacy FOOOCUS references
            return AIImageGeneratorMetadataParser()
...
def read_info_from_image(file) -> tuple[str | None, MetadataScheme | None]:
    ...
    metadata_scheme = items.pop('ai_image_generator_scheme', None)
    ...
    if isinstance(parameters, dict):
        metadata_scheme = MetadataScheme.AI_IMAGE_GENERATOR
...
def get_exif(metadata: str | None, metadata_scheme: str):
    ...
    # 0x0131 = Software
    exif[0x0131] = 'AI Image Generator v' + image_generator_version.version
    # 0x927C = MakerNote
