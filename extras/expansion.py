# AI Image Generator GPT2 Expansion
# Algorithm created by Lvmin Zhang at 2023, Stanford
# If used inside AI Image Generator, any use is permitted.
# If used outside AI Image Generator, only non-commercial use is permitted (CC-By NC 4.0).
...
class AIImageGeneratorExpansion:
    def __init__(self):
        ...
        negative_words = open(os.path.join(path_fooocus_expansion, 'negative.txt'),
                              encoding='utf-8').read().strip().split('\n')
        self.bad_words_ids = [[self.tokenizer.encode(word, add_special_tokens=False)[0]] for word in negative_words]
        debug_list = [self.tokenizer.decode([i]) for i in self.tokenizer.encode(positive_words[0], add_special_tokens=False)]
        print(f'AI Image Generator V2 Expansion: Vocab with {len(debug_list)} words.')
        ...
        print(f'AI Image Generator Expansion engine loaded for {load_device}, use_fp16 = {use_fp16}.')
    ...
    def __call__(self, prompt, seed):
        ...
        if self.patcher.current_device != self.patcher.load_device:
            print('AI Image Generator Expansion loaded by itself.')
